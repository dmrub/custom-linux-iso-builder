...
cleanup() {
    ...
    if [[ -n "$ISO_MOUNT_PATH" ]]; then
        ...
        echo "Deleting $ISO_MOUNT_PATH"
        { chmod -R u+rw "$ISO_MOUNT_PATH" && rm -rf "$ISO_MOUNT_PATH"; } || true;
    fi
    if [[ -n "$EFIBOOT_MOUNT_PATH" ]]; then
        ...
        echo "Deleting $EFIBOOT_MOUNT_PATH"
        { chmod -R u+rw "$EFIBOOT_MOUNT_PATH" && rm -rf "$EFIBOOT_MOUNT_PATH"; } || true;
    fi
    if [[ -n "$DISK_DIR" ]]; then
...
if command -v xorriso >/dev/null 2>&1; then
    xorriso -osirrox on -indev "$MFILE" -extract / "$MDIR"
else
    fatal "Unsupported"
...
if command -v rsync >/dev/null 2>&1; then
    ...
    rsync --progress -av "$ISO_MOUNT_PATH/" "$DISK_DIR/"
else
    #sudo cp -pR "$ISO_MOUNT_PATH"/* "$DISK_DIR"
    cp -pR "$ISO_MOUNT_PATH"/* "$DISK_DIR"
fi

#sudo chown "$(id -u):$(id -g)" -R "$DISK_DIR"
chown "$(id -u):$(id -g)" -R "$DISK_DIR"
...
if [[ -n "$DEST_EFIBOOT_IMAGE_FILE" ]]; then
    ...
    DISK_EFIBOOT_IMAGE_FILE=$MFILE
    #file "$MFILE" #???EFI CHECK
    #cp "$MFILE" "$THIS_DIR/efi_disk.img" #???EFI CHECK
    #exit  #???EFI CHECK
...
(
    ...
    if [[ -n "$DISK_EFIBOOT_IMAGE_FILE" ]]; then
        if mdir -i "$DISK_EFIBOOT_IMAGE_FILE" "::/$DEST_GRUB_CFG_FILE"; then
            echo "Original grub.cfg $(mcopy -i "$DISK_EFIBOOT_IMAGE_FILE" "::/$DEST_GRUB_CFG_FILE" - | md5sum)"
            mcopy -Qo -i "$DISK_EFIBOOT_IMAGE_FILE" "$DISK_DIR/$DEST_GRUB_CFG_FILE" "::/$DEST_GRUB_CFG_FILE"
            echo "Modified grub.cfg $(mcopy -i "$DISK_EFIBOOT_IMAGE_FILE" "::/$DEST_GRUB_CFG_FILE" - | md5sum)"
        else
            echo "No $DEST_GRUB_CFG_FILE in EFI boot file"
        fi
        echo "Modified EFI boot file $DEST_EFIBOOT_IMAGE_FILE $(md5sum "$DISK_EFIBOOT_IMAGE_FILE")"
    fi
    if [[ -n "$EFIBOOT_MOUNT_PATH" ]]; then
        fatal "sudo"
